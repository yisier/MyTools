    ...
    <el-form>
        <el-form-item>
            <el-radio v-model='radioValue' :label="1">
                {{ $t('javaCron.day.dayPlaceholder') }}
            </el-radio>
        </el-form-item>

        <el-form-item>
            <el-radio v-model='radioValue' :label="2">
                {{ $t('javaCron.day.unspecified') }}
            </el-radio>
        </el-form-item>

        <el-form-item>
            <el-radio v-model='radioValue' :label="3">
                {{ $t('javaCron.day.cycleStart') }}
                <el-input-number v-model='cycle01' :min="1" :max="30" /> -
                <el-input-number v-model='cycle02' :min="cycle01 + 1" :max="31" />
                {{ $t('javaCron.day.toDay') }}
            </el-radio>
        </el-form-item>

        <el-form-item>
            <el-radio v-model='radioValue' :label="4">
                {{ $t('javaCron.day.averageStart') }}
                <el-input-number v-model='average01' :min="1" :max="30" /> {{ $t('javaCron.day.startSuffix') }}
                <el-input-number v-model='average02' :min="1" :max="31 - average01" />
                {{ $t('javaCron.day.executeEvery') }}
            </el-radio>
        </el-form-item>

        <el-form-item>
            <el-radio v-model='radioValue' :label="5">
                {{ $t('javaCron.day.monthlyOn') }}
                <el-input-number v-model='workday' :min="1" :max="31" />
                {{ $t('javaCron.day.lastWorkday') }}
            </el-radio>
        </el-form-item>

        <el-form-item>
            <el-radio v-model='radioValue' :label="6">
                {{ $t('javaCron.day.endOfMonth') }}
            </el-radio>
        </el-form-item>

        <el-form-item>
            <el-radio v-model='radioValue' :label="7">
                {{ $t('javaCron.day.specify') }}
                <el-select clearable v-model="checkboxList" :placeholder="$t('javaCron.specifyPlaceholder')"
                    multiple :multiple-limit="10">
