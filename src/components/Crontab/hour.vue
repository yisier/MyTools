    ...
    <el-form>
        <el-form-item>
            <el-radio v-model='radioValue' :label="1">
                {{ $t('javaCron.hour.hourPlaceholder') }}
            </el-radio>
        </el-form-item>

        <el-form-item>
            <el-radio v-model='radioValue' :label="2">
                {{ $t('javaCron.hour.cycleStart') }}
                <el-input-number v-model='cycle01' :min="0" :max="22" /> -
                <el-input-number v-model='cycle02' :min="cycle01 + 1" :max="23" />
                {{ $t('javaCron.hour.toHour') }}
            </el-radio>
        </el-form-item>

        <el-form-item>
            <el-radio v-model='radioValue' :label="3">
                {{ $t('javaCron.hour.averageStart') }}
                <el-input-number v-model='average01' :min="0" :max="22" /> {{ $t('javaCron.hour.startSuffix')  }}
                <el-input-number v-model='average02' :min="1" :max="23 - average01" />
                {{ $t('javaCron.hour.executeEveryHour') }}
            </el-radio>
        </el-form-item>

        <el-form-item>
            <el-radio v-model='radioValue' :label="4">
                {{ $t('javaCron.hour.specify') }}
