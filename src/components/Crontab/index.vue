<template>
    <div>
        <el-tabs type="border-card">
            <el-tab-pane :label="$t('javaCron.index.second')" v-if="shouldHide('second')">
                <CrontabSecond @update="updateCrontabValue" :check="checkNumber" :cron="crontabValueObj"
                    ref="cronsecond" />
            </el-tab-pane>

            <el-tab-pane :label="$t('javaCron.index.minute')" v-if="shouldHide('min')">
                <CrontabMin @update="updateCrontabValue" :check="checkNumber" :cron="crontabValueObj" ref="cronmin" />
            </el-tab-pane>

            <el-tab-pane :label="$t('javaCron.index.hour')" v-if="shouldHide('hour')">
                <CrontabHour @updateupdate="CrontabValue" :check="checkNumber" :cron="crontabValueObj" ref="cronhour" />
            </el-tab-pane>

            <el-tab-pane :label="$t('javaCron.index.day')" v-if="shouldHide('day')">
                <CrontabDay @update="updateCrontabValue" :check="checkNumber" :cron="crontabValueObj" ref="cronday" />
            </el-tab-pane>

            <el-tab-pane :label="$t('javaCron.index.month')" v-if="shouldHide('month')">
                <CrontabMonth @update="updateCrontabValue" :check="checkNumber" :cron="crontabValueObj"
                    ref="cronmonth" />
            </el-tab-pane>

            <el-tab-pane :label="$t('javaCron.index.week')" v-if="shouldHide('week')">
                <CrontabWeek @update="updateCrontabValue" :check="checkNumber" :cron="crontabValueObj" ref="cronweek" />
            </el-tab-pane>

            <el-tab-pane :label="$t('javaCron.index.year')" v-if="shouldHide('year')">
        ...
        <div class="popup-main">
            <div class="popup-result">
                <p class="title">{{ $t('javaCron.index.timeExpression') }}</p>
                <table>
                    <thead>
                        <th v-for="item of tabTitles" :key="item">{{ item }}</th>
                        <th>{{ $t('javaCron.index.corn') }}</th>
                    </thead>
                    <tbody>
                        <td>
...
const tabTitles = ref([t('javaCron.index.second'), t('javaCron.index.minute'), t('javaCron.index.hour'), t('javaCron.index.day'), t('javaCron.index.month'), t('javaCron.index.week'), t('javaCron.index.year')])
...
// 使用 watchEffect 监听 props.expression 的变化
watchEffect(() => {
    if (props.expression) {
        resolveExp()
    }
