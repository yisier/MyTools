    ...
    <el-form>
        <el-form-item>
            <el-radio v-model='radioValue' :label="1">
                {{ $t('javaCron.min.minutePlaceholder') }}
            </el-radio>
        </el-form-item>

        <el-form-item>
            <el-radio v-model='radioValue' :label="2">
                {{ $t('javaCron.min.cycleStart') }}
                <el-input-number v-model='cycle01' :min="0" :max="58" /> -
                <el-input-number v-model='cycle02' :min="cycle01 + 1" :max="59" />
                {{ $t('javaCron.min.toMinute') }}
            </el-radio>
        </el-form-item>

        <el-form-item>
            <el-radio v-model='radioValue' :label="3">
                {{ $t('javaCron.min.averageStart') }}
                <el-input-number v-model='average01' :min="0" :max="58" /> {{ $t('javaCron.min.startSuffix') }}
                <el-input-number v-model='average02' :min="1" :max="59 - average01" />
                {{ $t('javaCron.min.executeEveryMinute') }}
            </el-radio>
        </el-form-item>

        <el-form-item>
            <el-radio v-model='radioValue' :label="4">
                {{ $t('javaCron.min.specify') }}
