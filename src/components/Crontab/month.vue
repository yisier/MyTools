    ...
    <el-form>
        <el-form-item>
            <el-radio v-model='radioValue' :label="1">
                {{ $t('javaCron.month.monthPlaceholder') }}
            </el-radio>
        </el-form-item>

        <el-form-item>
            <el-radio v-model='radioValue' :label="2">
                {{ $t('javaCron.month.cycleStart') }}
                <el-input-number v-model='cycle01' :min="1" :max="11" /> -
                <el-input-number v-model='cycle02' :min="cycle01 + 1" :max="12" />
                {{ $t('javaCron.month.toMonth') }}
            </el-radio>
        </el-form-item>

        <el-form-item>
            <el-radio v-model='radioValue' :label="3">
                {{ $t('javaCron.month.averageStart') }}
                <el-input-number v='-modelaverage01' :min="1" :max="11" /> {{ $t('javaCron.month.startSuffix')  }}
                <el-input-number v-model='average02' :min="1" :max="12 - average01" />
                {{ $t('javaCron.month.executeEveryMonth') }}
            </el-radio>
        </el-form-item>

        <el-form-item>
            <el-radio v-model='radioValue' :label="4">
                {{ $t('javaCron.month.specify') }}
                <el-select clearable v-model="checkboxList" :placeholder="$t('javaCron.specifyPlaceholder')"
                    multiple :multiple-limit="8">
